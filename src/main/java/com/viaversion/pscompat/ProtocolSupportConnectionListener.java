...
public final class ProtocolSupportConnectionListener extends Connection.PacketListener {
    ...
    public ProtocolSupportConnectionListener(final Object connection) {
    ...
    public void onPacketReceiving(final Connection.PacketListener.PacketEvent event) {
        try {
            ...
            if (HANDSHAKE_PACKET_CLASS.isInstance(event.getPacket()) && GET_VERSION_METHOD.invoke(connection) == PROTOCOL_VERSION_MINECRAFT_FUTURE) {
                ...
                if (protocolVersion == Via.getAPI().getServerVersion().lowestSupportedProtocolVersion().getVersion()) {
